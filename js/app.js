...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 function learning(q,ok){
  ...
  <figure class="concept-figure"><figcaption><div><span class="eyebrow">VISUAL EXPLANATION</span><h3>${esc(l.title)}</h3></div>${btn('Enlarge visual','enlarge','secondary',`data-lesson="${q.lessonId}"`)}</figcaption><a href="assets/lessons/${q.lessonId}.svg?v=20260924.3" target="_blank" rel="noopener" aria-label="Open ${esc(l.title)} diagram at full size"><picture><source media="(max-width: 600px)" srcset="assets/lessons/${q.lessonId}-mobile.svg?v=20260924.3"><img src="assets/lessons/${q.lessonId}.svg?v=20260924.3" alt="${esc(l.nodes.map(n=>n.title+': '+n.detail).join('. '))}" loading="lazy"></picture></a><p class="visual-caption">Original teaching diagram, not an AWS console screenshot. Read the ${l.layout==='comparison'?'panels as a comparison':'numbered steps with the explanation'}.</p></figure>
 ...
 main.addEventListener('click',e=>{
  ...
  if(action==='enlarge'){const l=window.LESSONS[b.dataset.lesson];document.querySelector('#visual-content').innerHTML=`<h2>${esc(l.title)}</h2><img src="assets/lessons/${b.dataset.lesson}.svg?v=20260924.3" alt="${esc(l.nodes.map(n=>n.title+': '+n.detail).join('. '))}">`;document.querySelector('#visual-dialog').showModal();return;}
